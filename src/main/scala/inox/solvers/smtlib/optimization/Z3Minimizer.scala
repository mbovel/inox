...
trait Z3Minimizer extends Z3Optimizer {
  ...
  private def sizersOf(e: Expr): Seq[Expr] = {
    /**
    ...
    def adtSizers(ctors: Seq[TypedADTConstructor]): Seq[Expr] = {
      ...
      def getZeroLiterals(field: ValDef): Seq[Expr] = field.tpe match {
        case BVType(signed, size) => Seq(BVLiteral(signed, 0, size))
        case adt @ ADTType(_, _) =>
          adt.lookupSort match {
            case Some(tsort) => tsort.constructors.flatMap(_.fields).distinct.flatMap(getZeroLiterals)
            case None => Seq()
          }
      ...
      def fieldSizer(field: ValDef)(ctor: TypedADTConstructor): Seq[Expr] = {
        if (ctor.fields.contains(field)) sizersOf(ADTSelector(e, field.id)) else getZeroLiterals(field)
      }
      // BV32 for the size, because the number of fields can't be larger anyway (since fields.length is an int),
      ...
      val fieldSizes = ctors.flatMap(_.fields).distinct.flatMap(f => rec(ctors, getZeroLiterals(f), fieldSizer(f)))
      adtSize ++ fieldSizes
    }
  ...
  override def assertCnstr(expr: Expr): Unit = {
    for (freeVar <- variablesOf(expr) ; toMinimize <- sizersOf(freeVar)) {
      minimize(toMinimize)
    }
